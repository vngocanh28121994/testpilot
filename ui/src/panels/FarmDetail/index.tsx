...
const PAGE_DESCRIPTION = 'Một lượt chạy trên thiết bị thật của AWS: tiến trình, report, ảnh và video.';
...
export default function FarmDetailPanel({ runId }: { runId: string }) {
  ...
  if (!run) return <AppShell title={navTitle('device-farm')} description={PAGE_DESCRIPTION}><Link to="/farm" className="text-sm underline">← Quay lại</Link><p className="text-muted-foreground mt-4">{state.isPending ? 'Đang tải…' : 'Không tìm thấy lần chạy.'}</p></AppShell>;
