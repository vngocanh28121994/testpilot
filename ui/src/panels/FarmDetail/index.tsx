...
function ReportCard({ report }: { report: ReportView }) { return <section className="border-border bg-card mt-4 rounded-lg border p-4"><div className="flex flex-wrap gap-2"><b>{report.device ?? report.id}</b><StatusPill status={report.status}/>{report.counters && <span className="text-muted-foreground text-sm">{report.counters.passed} passed · {report.counters.failed} failed</span>}<a className="text-sm underline" href={report.url} target="_blank" rel="noreferrer">Báo cáo</a></div>{report.videoUrls?.map((url) => <RunVideo key={url} url={url} report={report}/>) }{report.hasLog && <LazyLog url={`${ROUTES.runLog}?id=${encodeURIComponent(report.id)}`} label={`Log ${report.device ?? report.id}`} className="max-h-64"/>}</section>; }
