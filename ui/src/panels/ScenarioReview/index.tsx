...
function ReviewBody({ state, search }: { state: StateResponse; search: ScenarioSearch }) {
  ...
  const applyFilters = (form: HTMLFormElement) => {
    ...
    const tags = data.getAll('tags').filter((tag): tag is string => typeof tag === 'string' && tag.length > 0);
    void navigate({ search: { q: String(data.get('q') || '') || undefined, file: String(data.get('file') || '') || undefined, status: (String(data.get('status') || '') || undefined) as ScenarioSearch['status'], tags: tags.length > 0 ? tags : undefined, page: undefined } });
  };
  ...
  return <AppShell title="Kịch bản" actions={<Link to="/scenarios" search={{}} className="text-xs underline">Xoá bộ lọc</Link>}>
    <form className="border-border grid gap-2 rounded-lg border p-4 md:grid-cols-5" onSubmit={(event) => { event.preventDefault(); applyFilters(event.currentTarget); }}>
      ...
      <select name="tags" multiple defaultValue={search.tags ?? []} aria-label="Lọc theo tag" className="border-border bg-background min-h-10 rounded border p-2 text-sm"><option value="">Tất cả tag</option>{tagOptions.map((tag) => <option key={tag}>{tag}</option>)}</select>
